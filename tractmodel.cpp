...
TractModel::TractModel(BrainModel & brain, const QString & name, const TractData & tract)
	: brain(brain),
      ...
      name(name)
{
	tract_cell_create(&identity, &tract);
	flags = (tractFlags) 0L;
	gene = 0L;
	mirror = 0L;
}
...
{
	QString lobe;
	lobe.sprintf("%.16s", identity.data.data_source);
	return lobe == it->name;
}
...
{
	QString lobe;
	lobe.sprintf("%.16s", identity.data.origin_lobe);
	return lobe == it->name;
}
...
int TractModel::Dendrite::dataSource() const
{
	if(!_mode)
	{
		return data_source_cell_from_origin_lobe_synapse(
			&self.identity, dendrite / self.identity.data.origin_lobe_max_synapses,
			synapse()) + self.identity.data.data_source_first;
	}

	return _dendrite / self.identity.data.data_source_max_synapses + self.identity.data.data_source_first;
}
...
int TractModel::Dendrite::originCell() const
{
	if(!_mode)
	{
		return dendrite / self.identity.data.origin_lobe_max_synapses + self.identity.data.origin_lobe_first;
	}

	return origin_lobe_cell_from_data_source_synapse(
		&self.identity, _dendrite / self.identity.data.data_source_max_synapses,
		synapse()) + self.identity.data.origin_lobe_first;
}
...
int TractModel::Dendrite::synapse() const
{
	if(!_mode)
	{
		return dendrite % self.identity.data.origin_lobe_max_synapses;
	}
	else
	{
		return _dendrite % self.identity.data.data_source_max_synapses;
	}
}
...
void TractModel::Dendrite::popFront()
{
	...
	do{
		++_dendrite;
		dendrite = get_synapse_from_data_source(&self.identity, _dendrite);
...
/*
	...
	tract_cell_create(&identity, &data);
	tract_cell_set_origin_lobe(&identity, &origin_lobe->identity);
	tract_cell_set_data_source(&identity, &data_source->identity);
/*
	if(mirror)
	{
		tract_cell_set_mirror(&identity, &mirror->identity);
	}*/
	tract_cell_initialize(&identity);
}
...
bool TractModel::getLobes(LobeModel *& origin_lobe, LobeModel *& data_source, const char origin[16], const char source[16]) const
{
	QString origin_lobe_name;
	origin_lobe_name.sprintf("%.16s", origin);

	QString data_source_name;
	data_source_name.sprintf("%.16s", source);
...
{
	return getLobes(origin_lobe, data_source, identity.data.origin_lobe, identity.data.data_source);
}
...
void TractModel::propagateHighlightForward(std::vector<uint16_t> & light)
{
	...
	{
		for(auto syn = Dendrite(*this, *i - identity.data.data_source_first, true); !syn.empty() && syn.dataSource() == *i; syn.popFront())
		{
...
void TractModel::propagateHighlightBackward(std::vector<uint16_t> & light)
{
	...
	for(auto i = light.begin(); i != light.end(); ++i)
	{
		for(auto syn = Dendrite(*this, *i - identity.data.origin_lobe_first, false); !syn.empty() && syn.originCell() == *i; syn.popFront())
		{
			additional.push_back(syn.number());
